...
import com.namacmo.paymentservice.v1.domain.PaymentEventMessagePublisher
...
class R2DBCPaymentStatusUpdateRepository (
  ...
  private fun checkPreviousPaymentOrderStatus(orderId: String): Mono<List<Pair<String, String>>> {
  ...
  private fun selectPaymentOrderStatus(orderId: String): Flux<Pair<String, String>> {
    return databaseClient.sql(SELECT_PAYMENT_ORDER_STATUS_QUERY)
      .bind("orderId", orderId)
      .fetch()
      .all()
      .map { it["id"] as String to it["payment_order_status"] as String}
  }

  private fun insertPaymentHistory(
    paymentOrderIdToStatus: List<Pair<String, String>>,
    ...
    val valuesClauses = paymentOrderIdToStatus.joinToString(", ") {
      "('${UUID.randomUUID()}', '${it.first}', '${it.second}', '${status}', '${reason}' )"
    }
  ...
  private fun updatePaymentEventExtraDetails(command: PaymentStatusUpdateCommand): Mono<Long> {
    return databaseClient.sql(UPDATE_PAYMENT_EVENT_EXTRA_DETAILS_QUERY)
      ...
      .bind("method", command.extraDetails.methodGroup.name)
  ...
  companion object {
    ...
    val INSERT_PAYMENT_HISTORY_QUERY = fun (valueClauses: String) = """
      INSERT INTO payment_order_histories (id, payment_order_id, previous_status, new_status, reason)
      VALUES $valueClauses
    """.trimIndent()
